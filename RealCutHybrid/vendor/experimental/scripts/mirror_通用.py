...
the original source audio and never borrows voice from the external sound
library, so multi-person videos do not get another host's voice mixed in.
Mirror/reverse fills use a silent audio track.
"""
...
def make_silent_wav(path, duration_us):
    """Create a short PCM WAV silence file used only for picture-fill segments."""
    sample_rate = 44100
    channels = 1
    sampwidth = 2
    duration_s = max(0.1, duration_us / 1e6)
    ...
    return int(n_frames * 1000000 / sample_rate)
...
def main(dp_str, force=False, use_reverse=True):
    ...
    for i in range(need):
        ...
        src_s, src_e = clothes_src_area[i % len(clothes_src_area)]
        fill_ms = min(max(int(src_e - src_s), 1500), FORCE_FILL_MS if force else 5000)
        audio_dur_us = min(fill_ms * 1000, remaining_us)
        if audio_dur_us < 500000:
            print('[warn] not enough timeline budget, fill stopped')
            break
        print(f'  fill{i + 1}: silent audio ({audio_dur_us / 1e6:.1f}s) [picture fill only]')

        audio_name = f'mirror_fill_{uuid.uuid4().hex[:8]}.wav'
        audio_path = os.path.join(str(dp), audio_name)
        actual_audio_us = make_silent_wav(audio_path, audio_dur_us)
        audio_mat_id = uid()
    ...
    print(f'added {len(new_seg_metas)} segments (video only + silent audio)')
