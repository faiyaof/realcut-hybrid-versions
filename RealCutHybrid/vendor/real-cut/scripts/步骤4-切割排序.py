...
CATEGORY_MAX = {'爆点': 1, '展示衣服': 20, '金句': 1, '价格': 1}
MAX_VIDEO_DURATION_MS = 30000  # 成片最长不超过30秒

def _locate_clip_lib():
    """定位素材库：优先脚本同级的 ../素材库，其次几个已知位置，最后回退旧路径。"""
    candidates = [
        Path(r'C:\Users\JT\Documents\剪辑\爆点+金句 素材库\爆点素材库\素材库'),
        Path(__file__).resolve().parent.parent / "素材库",
    ]
    for c in candidates:
        try:
...
CLIP_LIB = _locate_clip_lib()
print(f'素材库路径: {CLIP_LIB}')
...
- 金句（第4段，最多1条）：名人名言、价值观输出、认同感、品质金句、人生道理（可选，不够15s才加）
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if resp.status_code != 200:
                print(f' 分类API失败 (尝试 {attempt+1}): {resp.status_code}')
...
def get_audio_duration_ms(fp):
    try:
        r = subprocess.run(['ffprobe', '-v', 'quiet', '-show_entries', 'format=duration', '-of', 'csv=p=0', str(fp)], capture_output=True, text=True, timeout=10)
...
def pick_best_clip(clips, sentences):
    """从素材库候选中按“最匹配视频主题”挑选：
    用 clip 文件名的中文关键片段与视频口播文本做 2-gram 重叠打分，最高者胜；都不匹配则随机。"""
    import random
    if not clips:
        return None
    video_text = ''.join(s.get('text', '') for s in sentences)
    if not video_text:
        return random.choice(clips)
    best, best_score = None, -1
    for c in clips:
        keys = re.findall(r'[\u4e00-\u9fff]{2,}', c.stem)
        score = 0
        for k in keys:
            if k in video_text:
                score += 2
            for i in range(len(k) - 1):
                if k[i:i + 2] in video_text:
                    score += 1
        if score > best_score:
            best_score, best = score, c
    if best_score <= 0:
        return random.choice(clips)
    print(f'   素材库主题匹配: {best.name} (得分{best_score})')
    return best

def enforce_limits_and_fallback(grouped, sentences, asr_source_audio, pre_duration_ms=0, discarded=None):
    categorize_price_sentences(grouped, sentences)
    for cat in ['爆点','展示衣服','金句','价格']:
        ...
        if cat == '爆点' and len(grouped[cat]) == 0:
            if discarded is not None:
                ...
                if best_idx is not None:
                    ...
                    continue
            clips = scan_folder_clips(CLIP_LIB / cat)
            if clips:
                chosen = pick_best_clip(clips, sentences)
                grouped[f'_{cat}_fallback'] = [chosen]
                dur_ms = get_audio_duration_ms(chosen)
                print(f' {cat}: 视频无匹配内容，从素材库补充 {chosen.name} ({dur_ms/1000:.1f}s)')
    # 金句判定：移到排序后用实际总时长，这里不处理
    return grouped
...
def build_ordered_segments(grouped, sentences):
    ...
    for cat in order:
        ...
        for fp in grouped.get(f'_{cat}_fallback', []):
            dur_ms = get_audio_duration_ms(fp)
            ordered.append({'category': cat, 'text': f'[素材库] {fp.stem}', 'src_start_ms': 0, 'src_end_ms': dur_ms, 'src_dur_ms': dur_ms, 'source': 'file', 'file': str(fp)})
    return ordered

def limit_segments_to_max_duration(segs, max_ms=MAX_VIDEO_DURATION_MS):
    """把成片总时长压到30秒内，同时尽量保留爆点/展示衣服/金句/价格结构。"""
    total = sum(int(s.get('src_dur_ms', 0)) for s in segs)
    if total <= max_ms:
        return segs
    ...
    for s in segs:
        cat = s.get('category', '')
        if cat in ('爆点', '金句', '价格') and cat not in seen:
            required.append(s)
            seen.add(cat)
        elif cat == '展示衣服' and cat not in seen:
            required.append(s)
            seen.add(cat)
    ...
    for s in segs:
        if s in required:
            keep.append(s)
        elif s.get('category') == '展示衣服':
            trial = sum(int(x.get('src_dur_ms', 0)) for x in keep) + int(s.get('src_dur_ms', 0))
            if trial <= max_ms:
                keep.append(s)
    keep.sort(key=lambda x: segs.index(x))

    total_keep = sum(int(s.get('src_dur_ms', 0)) for s in keep)
    if total_keep > max_ms:
        scale = max_ms / total_keep
        min_ms = 500
        for s in keep:
            old = int(s.get('src_dur_ms', 0))
            new = max(min_ms, int(old * scale))
            s['src_dur_ms'] = new
            s['src_end_ms'] = int(s.get('src_start_ms', 0)) + new

    # 如果仍超过30秒，优先删掉额外展示衣服段；最后再硬截最后一段。
    while sum(int(s.get('src_dur_ms', 0)) for s in keep) > max_ms and len(keep) > 1:
        extra = [i for i, s in enumerate(keep) if s.get('category') == '展示衣服']
        if len(extra) > 1:
            keep.pop(extra[-1])
            continue
        over = sum(int(s.get('src_dur_ms', 0)) for s in keep) - max_ms
        last = keep[-1]
        last['src_dur_ms'] = max(1, int(last.get('src_dur_ms', 0)) - over)
        last['src_end_ms'] = int(last.get('src_start_ms', 0)) + last['src_dur_ms']
...
def main(dp_str, auto_open=True):
    ...
    if segs:
        ...
        if not has_jinju:
            print('\n 无金句 -> 从素材库补充金句（选项B：金句必须保留）')
            clips = scan_folder_clips(CLIP_LIB)
            if clips:
                chosen = pick_best_clip(clips, sentences)
                dur_ms = get_audio_duration_ms(chosen)
                price_idx = next((i for i, x in enumerate(segs) if x['category'] == '价格'), len(segs))
                segs.insert(price_idx, {'category': '金句', 'text': f'[素材库] {chosen.stem}', 'src_start_ms': 0, 'src_end_ms': dur_ms, 'src_dur_ms': dur_ms, 'source': 'file', 'file': str(chosen)})
                print(f' 金句: 从素材库补充 {chosen.name} ({dur_ms/1000:.1f}s)')
            else:
                print(' 金句: 素材库为空，无法补充')
