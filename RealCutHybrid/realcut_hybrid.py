...
STEPS: list[StepSpec] = [
    ...
    StepSpec("font_unify", "字幕字体统一", "font_unify.py", 15.0, phase="style", no_open=True),
...
def step_enabled(step: StepSpec, opts: argparse.Namespace) -> bool:
    if opts.phase2 and step.key not in {"5_fade", "7_subtitles", "12_style", "style_apply", "audio_smooth", "bgm_normalize", "font_unify", "subtitle_gaps"}:
        return False
    ...
    if step.key in {"bgm_normalize", "font_unify"}:
...
def verify_step_output(
    ...
    elif step.key == "font_unify":
        data = _json_read(draft / "draft_content.json", {})
        if not _dynamic_fonts_unified(data):
            raise RuntimeError("字体统一验证失败：动态字幕字体不一致")
...
def run_step_once(
    ...
    if step.phase == "subtitle" and not opts.dry_run:
        if opts.no_close_jianying and jianying_running():
            raise RuntimeError("剪映正在运行；步骤7/12必须关闭剪映，或去掉 --no-close-jianying")
...
def run_task(opts: argparse.Namespace, video_path: Path) -> int:
    ...
    try:
        for step in STEPS:
            ...
            ok, next_draft, output = run_step_once(step, opts, state, video, draft)
            if step.key == "1_import":
                if opts.dry_run:
                    draft = DEFAULT_DRAFT_ROOT / video.stem
                else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    restyle_p.add_argument("--no-reorder", action="store_true", help="跳过主视频全轨随机重排（默认会更换开头）")
