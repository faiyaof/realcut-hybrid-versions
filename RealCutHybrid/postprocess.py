...
def mirror_and_reorder(
    ...
    """Mirror and reorder the main footage while leaving style overlays intact.

    Reordering covers the full main footage track so the opening shot changes.
    The applied seed is recorded to keep retries idempotent.
    """
    if dry_run:
    ...
    processed_seeds = data.setdefault("realcut_reorder_seeds", [])
    reorder_already_applied = seed in processed_seeds

    video_materials = {
        mat.get("id"): mat
        for mat in data.get("materials", {}).get("videos", []) or []
    ...
    for tr in data.get("tracks", []):
        ...
        if not segs:
            continue
        footage = [video_materials.get(seg.get("material_id")) for seg in segs]
        if not any(mat and mat.get("type") == "video" for mat in footage):
            continue
        ...
        if reorder and len(segs) >= 2 and not reorder_already_applied:
            old_first_id = segs[0].get("id")
            reordered = list(segs)
            rng.shuffle(reordered)
            if reordered[0].get("id") == old_first_id:
                swap_index = rng.randrange(1, len(reordered))
                reordered[0], reordered[swap_index] = reordered[swap_index], reordered[0]
            tr["segments"] = reordered
            opening_changed = tr["segments"][0].get("id") != old_first_id
            changed = True
    ...
    if reorder and not reorder_already_applied:
    ...
    reorder_status = "已跳过（相同种子已处理）" if reorder_already_applied else "完成"
    log(
        f"[postprocess] 主视频轨已镜像，重排={reorder_status}，"
        f"开头已更换={opening_changed}，段数={count}"
    )
    return True
