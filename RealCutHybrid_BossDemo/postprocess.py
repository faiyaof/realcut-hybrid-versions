...
STYLE_LIB = Path(os.environ.get("REALCUT_STYLE_LIB", str(ROOT / "assets" / "styles")))
...
def resolve_style_template(style: str, log: LogFn = _log_default) -> Path:
    ...
    if style:
        for cand in (STYLE_LIB / f"{style}模板", DEFAULT_DRAFT_ROOT / style):
...
def force_font(
    ...
    template = rewrite_pkg_asset_paths(template)
