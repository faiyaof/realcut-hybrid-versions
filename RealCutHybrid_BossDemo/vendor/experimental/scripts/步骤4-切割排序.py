...
def _locate_clip_lib():
    ...
    candidates = [
        Path(os.environ.get('REALCUT_BAODIAN_LIB', r'C:\Users\JT\Documents\剪辑\爆点+金句 素材库\爆点素材库\素材库')),
...
def build_ordered_segments(grouped, sentences):
    ...
    for fp in grouped.get('_爆点_fallback', []):
        dur_ms = get_audio_duration_ms(fp)
        rest.insert(0, {'category': '爆点', 'text': f'[素材库] {fp.stem}', 'src_start_ms': 0, 'src_end_ms': dur_ms, 'src_dur_ms': dur_ms, 'source': 'file', 'file': str(fp)})
    tail = []
    ...
    for fp in grouped.get('_金句_fallback', []):
        dur_ms = get_audio_duration_ms(fp)
        tail.append({'category': '金句', 'text': f'[素材库] {fp.stem}', 'src_start_ms': 0, 'src_end_ms': dur_ms, 'src_dur_ms': dur_ms, 'source': 'file', 'file': str(fp)})
    ...
    if segs:
        ...
        if not has_jinju:
            ...
            if clips:
                chosen = pick_best_clip(clips, sentences)
                dur_ms = get_audio_duration_ms(chosen)
                segs.append({'category': '金句', 'text': f'[素材库] {chosen.stem}', 'src_start_ms': 0, 'src_end_ms': dur_ms, 'src_dur_ms': dur_ms, 'source': 'file', 'file': str(chosen)})
