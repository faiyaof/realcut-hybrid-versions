...
CLIP_LIB_BASE = Path(os.environ.get('REALCUT_CLIP_LIB', r'C:\Users\JT\Documents\剪辑\爆点+金句 素材库'))
CLIP_LIB = str(CLIP_LIB_BASE / '爆点素材库' / '素材库' / '_audio')
if not os.path.isdir(CLIP_LIB):
    CLIP_LIB = str(CLIP_LIB_BASE / '_audio')
...
def pick_audio(exclude=None, max_dur_ms=None):
    ...
    candidates = [
        CLIP_LIB,
        str(CLIP_LIB_BASE / '金句'),
        str(CLIP_LIB_BASE / '爆点'),
    ]
    all_files = []
