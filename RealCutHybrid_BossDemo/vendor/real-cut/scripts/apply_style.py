...
"""
import json, sys, os, uuid, shutil, argparse, copy, io, random
...
def apply_style(dp_str, style_name=None, tmpl_path=None):
    ...
    if tmpl_path:
        ...
        tmpl_dir = STYLE_LIB / f'{style_name}模板'
        if not (tmpl_dir / 'draft_content.json').exists():
            tmpl_dir = DRAFT_ROOT / style_name
    else:
    ...
    if already_applied:
        ...
        for mat in draft.get('materials', {}).get('texts', []):
            if isinstance(mat.get('style_overlay_mark'), str):
                overlay_mark_ids.add(mat['id'])
        for mat in draft.get('materials', {}).get('stickers', []):
            if isinstance(mat.get('style_overlay_mark'), str):
                overlay_mark_ids.add(mat['id'])
        if overlay_mark_ids:
            # 移除被标记的素材
            for key in ['texts', 'stickers']:
                mats = draft.get('materials', {}).get(key, [])
                draft['materials'][key] = [m for m in mats if m['id'] not in overlay_mark_ids]
            # 移除引用了被标记素材的轨道段及其轨道
        ...
        _apply_overlay(draft, tmpl, total_dur, current_mark)
...
def _apply_overlay(draft, tmpl, total_dur, mark):
    """叠加模板固定元素（风格文字/贴纸/BGM），给素材/轨道写入唯一 mark 便于幂等清理。"""
    tmpl_mats = tmpl.get('materials', {})
    tmpl_tracks = tmpl.get('tracks', [])
    tmpl_texts = {t['id']: t for t in tmpl_mats.get('texts', [])}
    tmpl_stickers = {t['id']: t for t in tmpl_mats.get('stickers', [])}
    # 音频素材映射（区分 music BGM 与 sound 配音）
    tmpl_audios = {}
    for m in tmpl_mats.get('audios', []) + tmpl_mats.get('music', []):
        tmpl_audios[m['id']] = m
    overlay_mats = {'texts': [], 'stickers': []}
    overlay_tracks = []
    for tr in tmpl_tracks:
        if tr['type'] == 'video':
            continue
    ...
    for mat in overlay_mats['texts'] + overlay_mats['stickers']:
        mat['duration'] = total_dur
    # 写幂等标记：下次同风格直接跳过叠加
    draft['style_applied'] = mark
    print(f"[3/4] 已叠加 {len(overlay_mats['texts'])}风格文字 + {len(overlay_mats['stickers'])}贴纸")
