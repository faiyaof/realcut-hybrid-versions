...
def ai_segment_text(text, max_chars=10, max_retries=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 文本保真校验：AI 输出的字符应大部分在原文中（防止 AI 改写）
            _joined = ''.join(segs)
            ...
            if _fidelity < 0.85:
                ...
                return None, None
            # 过滤超长段（AI偶尔违反max_chars），用 jieba 补切
            final_segs = []
            for s in segs:
                if len(s) <= max_chars:
                    final_segs.append(s)
...
def chinese_num_to_arabic(text):
    ...
    special = {"六千九":"6900","九千二":"9200","四千六":"4600","十几万":"10几万","几十万":"几十万","几百":"几百","几千":"几千"}
    special["几十"] = "几十"
...
def ai_review_transcript(sentences):
    """对整段 ASR 结果做 AI 审核，修正错别字/数字/明显不通顺处，保持句子数量和顺序。"""
    key = os.environ.get('DASHSCOPE_API_KEY', '')
    if not key or not sentences:
        return None
    try:
        import dashscope
        from dashscope import Generation
...
def generate_subs_from_full_audio(dp, draft):
    ...
    for sent in full_sentences:
        sent['text'] = chinese_num_to_arabic(sent.get('text', ''))
        sent['text'] = fix_typos(sent['text'])
        sent['text'] = fix_known_golden_quote(sent['text'])
    reviewed = ai_review_transcript(full_sentences)
    subs = []
    ai_keywords_all = []
    ...
    for idx, sent in enumerate(full_sentences):
        ...
        text = chinese_num_to_arabic(s_text)
...
subs, ai_keywords_all = generate_subs_from_full_audio(dp, draft)
if subs is None:
