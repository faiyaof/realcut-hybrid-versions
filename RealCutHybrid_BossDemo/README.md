...
这是从 `RealCutHybrid` 复制出来的演示版，只保留固定自动化路径：选择单个视频或文件夹 → 完整剪辑 → 风格2 → 字幕空隙补齐。原程序不改动，演示版不开放并行和风格选择；文件夹会批量导入其中所有视频。

本项目是薄调度层：把已验证的 `real-cut` skill 原样复制到
...
RealCutHybrid/
  ...
  vendor/experimental/   当前实验引擎：60ms 淡入淡出、音频平滑、字幕审校、字幕空隙补齐
...
演示版默认使用 `http://127.0.0.1:8766`，避免和原版 8765 冲突。Web 页面支持：

- 选择单个视频或文件夹，批量加入持久化后台队列；Web 重启后会自动恢复未完成任务
  - 文件夹默认扫描子目录；取消“扫描文件夹子目录”后只导入所选目录内的视频
- 查看任务状态、步骤断点、进度和失败原因
- 查看运行日志、Markdown/JSON 报告和字幕复核清单
- 断点续跑、只重跑字幕阶段、从头重跑
- 提交即按固定参数执行：完整剪辑、风格2、音频平滑、字幕复核、字幕空隙补齐
- 一键“只补字幕空隙”：对已有草稿任务直接跑 `subtitle_gaps`，不重跑前面步骤

Web 服务通过 `realcut_hybrid.py run` 调用同一套 CLI，因此不会绕过已验证的调度

队列状态保存在 `web_queue.json`，包括待办和运行中任务；提交任务时先落盘再入队，所以 Web 进程重启不会再把排队任务丢光。演示版固定单任务顺序执行，不开放并行。
...
- 自动执行步骤 1-6、8、10、音频平滑、11、7、12、风格套用（固定风格2）、BGM 归一化、字幕空隙补齐；不自动执行字幕字体统一。
- 步骤 9 花字音效默认关闭。
- 步骤 11 水印默认关闭。
- BGM 默认使用序号 10。
...
1. real-cut 全流程（演示版固定风格2）：`batch`
2. 风格1转风格2 + 镜像重排：`restyle`
3. 可选字体工具，不进自动流水线：`force-font`
4. 字幕空隙补满：`fill-gaps`
...
`qwen:qwen-plus`）。完整语音轨和金句外部素材的 ASR 结果都会先做本地词表修正，再交给
同一个受限审校流程修错别字。
...
脚本仍保留在实验副本中，但本演示版调度器不会自动运行。它只处理动态字幕轨
`flag=1` 的 subtitle 素材，不修改 `flag=0` 的风格固定文字/贴纸元素。
