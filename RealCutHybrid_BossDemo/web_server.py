...
and always runs tasks one at a time for the boss demo.
"""
...
class TaskQueue:
    ...
    @staticmethod
    def _kill_process(proc: subprocess.Popen, task_id: str) -> None:
    ...
    def pause(self) -> bool:
        ...
        with self._cond:
            self._paused = True
            moved = list(self._running.values())
            procs = [self._processes.get(item.task_id) for item in moved if item.task_id in self._processes]
        ...
        for item in moved:
            append_log(item.task_id, "全局暂停：任务已放回队列头部，保留断点状态")
        for item, proc in zip(moved, procs):
            if proc is not None:
                self._kill_process(proc, item.task_id)
        return True
    ...
    def resume(self) -> bool:
        with self._cond:
            ...
            self._save_queue_locked()
            self._cond.notify_all()
        append_log("web_queue", "全局恢复：任务队列继续执行")
        return True
        ...
        while True:
            with self._cond:
                while (
                    (
                        not self._items
                        or len(self._running) >= self._max_concurrency
                        or self._paused
                    )
                    and not self._stop
                ):
    ...
    def _run_item(self, item: QueueItem) -> None:
        ...
        try:
            ...
            with self._cond:
                if not self._paused and item.task_id in self._running:
                    _force_state(item.task_id, "failed", str(exc))
            return

        with self._cond:
            paused = self._paused
            still_owned = item.task_id in self._running
            if not paused and still_owned:
                self._processes[item.task_id] = proc
                item.pid = proc.pid
                item.status = "running"
                self._save_queue_locked()
        if paused or not still_owned:
            append_log(item.task_id, "进程已因暂停/队列变更终止")
            self._kill_process(proc, item.task_id)
            return
...
def bootstrap_payload() -> dict:
    ...
    return {
        "app": {"name": "RealCut Auto", "version": "0.1.0", "platform": "windows"},
        "queue": queue,
        "tasks": ordered,
        "paths": {
            "root": str(ROOT),
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if path == "/api/queue/pause":
                task_queue.pause()
                return _send_json(self, {"ok": True, "paused": True})
            if path == "/api/queue/resume":
                task_queue.resume()
                return _send_json(self, {"ok": True, "paused": False})
            if path == "/api/browse":
                payload = self._json_body()
...
def serve(start_port: int, no_browser: bool = False) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="RealCut Auto Web")
    ...
    try:
        ...
        pass
    serve(args.port, no_browser=args.no_browser)
    return 0
