...
function renderPauseButton() {
  const btn = $('#pause-toggle-btn');
  if (!btn) return;
  const paused = Boolean(state.bootstrap?.queue?.paused);
  btn.innerHTML = `<i data-lucide="${paused ? 'play' : 'pause'}"></i><span>${paused ? '继续' : '暂停'}</span>`;
  btn.title = paused ? '继续任务队列' : '暂停任务队列';
...
function renderTasks() {
  ...
  const queuePaused = Boolean(state.bootstrap?.queue?.paused);
  $('#queue-caption').textContent = queuePaused ? '已全局暂停，可随时继续' : '单任务顺序执行';
  const runningSub = $('#metric-running')?.closest('.metric-card')?.querySelector('small');
  if (runningSub) runningSub.textContent = queuePaused ? '已全局暂停' : '单任务顺序执行';
...
function openJobModal() {
  ...
  if ($('#job-recursive')) $('#job-recursive').checked = true;
...
async function openPathPicker(mode, target, initial = '') {
  ...
  $('#path-modal-title').textContent = mode === 'video' ? '选择本地视频' : '选择素材文件夹';
  $('#path-picker-help').textContent = mode === 'video' ? '进入文件夹后单击视频完成选择' : '进入文件夹后点击“选择当前文件夹”';
...
function applyPickedPath(path) {
  ...
  $('#modal-video-name').textContent = picker.mode === 'folder' ? `${name}（文件夹）` : name;
  $('#modal-video-path').textContent = picker.mode === 'folder' ? `${path}（将导入其中所有视频）` : path;
  closePathPicker();
}
...
async function createJob() {
  ...
  const options = {
    bgm: 10,
    recursive: $('#job-recursive')?.checked !== false,
    snapshot_mode: 'json',
    max_attempts: 2,
    watermark: false,
    enable_flower_text: false,
    smooth_audio: true,
    review_subtitles: true,
    dry_run: false,
    no_close_jianying: false,
    no_restore: false,
    style: '风格2',
  };
...
document.addEventListener('click', async (event) => {
  ...
  if (event.target.closest('#pause-toggle-btn')) {
    try {
      const paused = Boolean(state.bootstrap?.queue?.paused);
      const data = await api(paused ? '/api/queue/resume' : '/api/queue/pause', { method: 'POST', body: '{}' });
      await refresh();
      toast(data.paused ? '已暂停，当前任务已保留，可从断点继续' : '已继续，任务队列开始执行');
    } catch (error) { toast(error.message); }
    return;
  }
